...
function parseJSONL(text) {
    ...
    rows.forEach(row => {
      ...
      columns.forEach(col => {
        ...
        if (col === 'Action') {
          ...
          if (['EC','ED','QA','MC-4','ES','EE','Overall Score'].includes(col) && val !== '-' && val !== undefined && val !== null) {
  ...
  function calcWeightedScore(row) {
    ...
    let score = 0;
    let total = 0;
    if (EC != null) { score += EC * w.EC; total += w.EC; }
    if (ED != null) { score += ED * w.ED; total += w.ED; }
    if (QA != null) { score += QA * w.QA; total += w.QA; }
    if (MC4 != null) { score += (MC4 * 20) * w.MC4; total += w.MC4; }
    if (ES != null) { score += (ES * 20) * w.ES; total += w.ES; }
    if (EE != null) { score += EE * w.EE; total += w.EE; }
  ...
  async function renderLeaderboard() {
    ...
    try {
      ...
      const columns = ['Rank', 'Team Name', 'Model', 'EC', 'ED', 'QA', 'MC-4', 'ES', 'EE', 'Overall Score', 'Submission Time', 'Action'];
  
      // Accessors map flexible keys to target columns
      const accessors = {
        'Rank': () => '',
        'Team Name': (r) => findValue(r, ['team', 'team_name', 'Team', 'TeamName']),
        ...
        'Overall Score': (r) => {
          ...
          if (calc != null) return toNumberOrDash(calc);
          const v = findValue(r, ['overall', 'Overall', 'overall_score', 'OverallScore', 'score']);
          return toNumberOrDash(v);
        },
        'Submission Time': (r) => {
        ...
        'Action': () => null, // handled specially in buildTableBody
        links: (r) => {
      ...
      Array.from(tbody.querySelectorAll('tr')).forEach((tr, idx) => {
        ...
        // Overall styling
        const overallIdx = columns.indexOf('Overall Score');
